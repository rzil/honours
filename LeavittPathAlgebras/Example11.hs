...
testHomomorphism1 = WLPA.wLPA_relations_present f weighted_example3 weighted_example
 where
  f (WLPA.AEdge "e" 1) = e1
  f (WLPA.AEdge "f" 1) = f1
  f (WLPA.AEdge "f" 2) = f2
...
testHomomorphism2 = WLPA.wLPA_relations_present f weighted_example0 weighted_example
 where
  f (WLPA.AVertex "v") = v + u
